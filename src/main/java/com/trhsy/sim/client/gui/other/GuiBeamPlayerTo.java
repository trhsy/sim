...
public class GuiBeamPlayerTo extends GuiScreen {
    ...
    public void drawScreen(int i, int j, float f) {
        try {
            ...
            StackTraceElement element=e.getStackTrace()[0];ModSimReloaded.log.error("GuiBeamPlayerTo-drawScreen出错了：" + e.getMessage()+"行数："+element.getLineNumber());
        }
    }
        ...
        try {
            if (guibutton.enabled) {
                if (guibutton.id == 0) {
                    ...
                    safePoint.addVector(safePoint.xCoord,safePoint.yCoord+1,safePoint.zCoord);
