 ...
 * @author Trhsy
 * @Package: com.trhsy.sim.network.client
 * @ClassName: PacketOpenMerchantsGui
 * @Description: 杂货商
 * @date 2023/07/24 下午 4:00
 */
public class PacketOpenMerchantsGui implements IMessage {
    public UUID id;
    public V3 v3;
    public PacketOpenMerchantsGui() {
    }

    public PacketOpenMerchantsGui(NpcData fd) {
    ...
    public static class Handler implements IMessageHandler<PacketOpenMerchantsGui, IMessage> {
        public Handler() {
        }
        @Override
        public IMessage onMessage(PacketOpenMerchantsGui message, MessageContext ctx) {
            try {
        ...
        private void handle(PacketOpenMerchantsGui message, MessageContext ctx) {
            ModSimClientLoader.openMerchants(message);
        }
    }
}
