    ...
    public static class Handler implements IMessageHandler<PacketSendTerrainType, IMessage> {
        ...
        public IMessage onMessage(PacketSendTerrainType message, MessageContext ctx) {
            try {
                ...
                ModSimLoader.log.error("PacketSendTerrainType出错了：" + var4.getMessage() + "行数：" + element.getLineNumber());
            }
        ...
        private void handle(PacketSendTerrainType message, MessageContext ctx) {
            ...
            ModSimLoader.getFolkDataByUID(message.folkID).job = new JobTerrainFormer(ModSimLoader.getFolkDataByUID(message.folkID), terrainType, V3.fromString(message.bPos).toBlockPos(),ctx.getServerHandler().player.world);
        }
    }
}
