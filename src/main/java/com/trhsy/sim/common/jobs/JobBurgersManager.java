...
public class JobBurgersManager extends Job {
    ...
    private void doPickup(String buildingSearch, Block pickUpItem, boolean doCompareMeta) {
        try {
            if (this.step == 1) {
                ...
                this.theFolk.forceMoveToXYZNoWarp((this.pickupBuildings.get(this.currentPickup)).primaryXYZ);
    ...
    private void stageHangingOut() {
        try {
            if (this.step % 2 == 0) {
                this.theFolk.forceMoveToXYZNoWarp(this.theStore.primaryXYZ);
            } else {
                this.theFolk.forceMoveToXYZNoWarp((V3) this.theStore.getSpecialBlocks(0).get(0));
            }
