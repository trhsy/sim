...
public class JobBrickMaker extends Job implements Serializable {
    ...
    private void stageCollectClay() {
        ...
        if (dist > 6.0D && System.currentTimeMillis() - this.lastGotocmd > 10000L) {
            ...
            try {
                ...
                this.gotoCount = 0;
                this.jobWorld.setBlock(this.blockOfClay.x.intValue(), this.blockOfClay.y.intValue(), this.blockOfClay.z.intValue(), Blocks.air, 0, 3);
    ...
    private void stageUseFurnace() {
        ...
        this.factoryChests = inventoriesFindClosest(this.theFolk.employedAt, 5);
        if (this.factoryFurnace == null) {
            ModSim.sendChat(this.theFolk.name + "："+I18n.format("container.sim.JobBrickMaker4"));
        } else {
            ...
            if (this.step == 1) {
                ...
                if (currentClay == null) {
                    ...
                    if (gotFuel == null) {
                        ModSim.sendChat(this.theFolk.name + I18n.format("container.sim.JobBrickMaker6"));
                        this.theStage = Stage.SCANFORCLAY;
                        this.step = 1;
                        return;
                    }
