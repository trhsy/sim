    ...
    public void respawnEntity(World world) {
        try {
            ...
            if (getDistanceToPlayer() < 100) {
                ...
                //设置实体在世界中的位置和偏航/俯仰
                V3 v=new V3(this.location.xCoord-0.5, this.location.yCoord+0.5, this.location.zCoord+0.5);
            ...
            if (now - this.timeSinceLastMinute > 60000L) {
                        ...
                        if (build != null && build.primaryXYZ != null && build.displayName.contains(clinic)) {
                            ...
                            if (this.destination == null) {
                                if (!build.blockSpecial.isEmpty()) {
                                    ...
                                    bed=new V3(bed.xCoord,bed.yCoord+0.5,bed.zCoord);
                                    forceMoveToXYZNoWarp(bed);
                                    //就要生孩子了,她正在去诊所的路上！
                ...
                if (this.action== FolkAction.ATHOME || this.action== FolkAction.STAYINGHOME) {
                    ...
                    if (liveAt != null) {
                        if (this.location.getDistanceTo(liveAt) > 5 && this.destination == null || this.location.theDimension != getHome().primaryXYZ.theDimension) {
                            ...
                            forceMoveToXYZNoWarp(new V3(liveAt.xCoord,liveAt.yCoord+0.5,liveAt.zCoord));
                        }
                    }
                }
                //如果他们在闲逛，就让他们逛商店和房子
                ...
                if (this.action== FolkAction.WANDER && isSpawned() && this.employedAt == null && this.age >= 18 && !this.statusText.contains(I18n.format("container.sim.folk_data.baby"))) {
                        ...
                        if (dist < 100) {
                            ...
                            if (b.type.contentEquals("commercial")) {
                                ...
                                if (hasShopKeeper) {
                                    ...
                                    //设置走过去
                                    V3 v=new V3(b.primaryXYZ.xCoord,b.primaryXYZ.yCoord+0.5,b.primaryXYZ.zCoord);
                                    forceMoveToXYZNoWarp(v);
                                ...
                                if (b.tenants != null && b.tenants.size() > 0) {
                                    ...
                                    if (!resy.name.contentEquals(name) && resy.hangingWith == null) {
                                        if (resy.action == FolkAction.WANDER || resy.action == FolkAction.STAYINGHOME) {
                                            ...
                                            V3 v1=new V3(b.primaryXYZ.xCoord,b.primaryXYZ.yCoord+0.5,b.primaryXYZ.zCoord);
                                            resy.forceMoveToXYZNoWarp(v1);
                                            if (this.destination != null) {
                                                //this.destination.doNotTimeout = true;
                                            }
                    ...
                    if (!gotWanderPoint) {
                        ...
                        //    //wanderTo.yCoord = wanderTo.yCoord+0.5;
                        //    wanderTo = new V3(wanderTo.xCoord, wanderTo.yCoord+0.5, wanderTo.zCoord);
                        //}

                        //ModSimReloaded.log.info("FolkData:onUpdate() 漫游命令 " + name + " to " + wanderTo.toString());
                        forceMoveToXYZNoWarp(wanderTo);
                        if (this.destination != null) {
                            //this.destination.doNotTimeout = true;
                        }
                    ...
                    if (male != null) {
                        forceMoveToXYZNoWarp(male.location);
                    }
                }
            ...
            if (now - this.timeSinceLastStatusUpdate > 1000L) {
                ...
                if (ModSimReloaded.isDayTime() || isNightOwl()) {
                    ...
                    if (this.employedAt != null && this.action!= FolkAction.ONWAYTOWORK && this.action!= FolkAction.ATWORK && this.pregnancyStage == 0.0F) {
                        ...
                        temp = new V3(temp.xCoord, temp.yCoord+0.5, temp.zCoord);
                        forceMoveToXYZNoWarp(temp);
//                        forceMoveToXYZNoWarp(temp, null);
...
//                        forceMoveToXYZNoWarp(temp, null);
                ...
                if (this.action == FolkAction.ONWAYTOWORK) {
                    ...
                    if (this.destination == null) {
                        ...
                        temp = new V3(temp.xCoord, temp.yCoord+0.5, temp.zCoord);
                        forceMoveToXYZNoWarp(temp);
//                        this.forceMoveToXYZNoWarp(temp, null);
                    }
                }
...
//                    forceMoveToXYZNoWarp(v3, GotoMethod.SHIFT);
                    forceMoveToXYZNoWarp(v3);
            ...
            if (this.gender == 1 && this.pregnancyStage == 0.0F) {
                ...
                if (malePartner != null && this.action== FolkAction.ATHOME && malePartner.action== FolkAction.ATHOME) {
                    ...
                    if (malePartner.isSpawned()) {
                        forceMoveToXYZNoWarp(new V3(malePartner.theEntity.posX, malePartner.theEntity.posY+1, malePartner.theEntity.posZ, malePartner.theEntity.dimension));
                    }
                }
            } else {
                this.matingStage = -1;
...
//            if (this.action== FolkAction.WANDER&&this.status2.equals(I18n.format("container.sim.folkData3"))) {
                ...
                if(!falg){
                    for (int b = 0; b < theBuilding.size(); b++) {
                        Building building = theBuilding.get(b);
                        if (building.tenants.size() == 0 && building.buildingComplete == true && building.type.contentEquals("residential")) {
                            building.tenants.add(this.name);
                            this.action= FolkAction.GOINGHOME;
                            this.actionArrival = FolkAction.STAYINGHOME;
                            V3 v3 = new V3(building.primaryXYZ.xCoord, building.primaryXYZ.yCoord+0.5, building.primaryXYZ.zCoord, building.primaryXYZ.theDimension);
                            forceMoveToXYZNoWarp(v3);
...
//                this.theEntity.isDead = false;
        ...
        try {
            ...
            for (int go = 30; go > 1; go--) {
                ...
                while (!found) {
                    ...
                    ret = new V3(ret.xCoord, ret.yCoord+0.5, ret.zCoord);
                    if (ret.yCoord > 200) {
                        break;
                    }
                }
    ...
    public boolean forceMoveToXYZNoWarp(V3 v3) {
        try {
            ...
            StackTraceElement element = e.getStackTrace()[0];
            ModSimReloaded.log.error("forceMoveToXYZNoWarp出错了：" + e.getMessage() + "行数：" + element.getLineNumber());
        }
        return false;
    }
    ...
    public void beamMeTo(V3 whereToIn) {
        try {
            ...
            for (int i = 0; i < 200; i++) {
                ...
                Block id2 = destWorld.getBlockState(new BlockPos(whereTo.xCoord, whereTo.yCoord+0.5, whereTo.zCoord)).getBlock();
                if (id1 == Blocks.air && id2 == Blocks.air) {
                    break;
                }
                whereTo = new V3(whereTo.xCoord, whereTo.yCoord+0.5, whereTo.zCoord);
                //whereTo.yCoord = whereTo.yCoord+0.5;
            }
        ...
        try {
            if (System.currentTimeMillis() - this.timeStartedGotoing > 4000L || this.beamingTo == null) {
                ...
                if (this.theEntity != null) {
                    ...
                    this.theEntity.setPositionAndUpdate(this.beamingTo.xCoord, this.beamingTo.yCoord + 0.5, this.beamingTo.zCoord);
        ...
        try {
            if (vocation != null) {
                ...
                if (vocation == Vocation.BUILDER) {
                    ...
                    this.theirJob = new JobLivestockFarmer(this);
                    //养兔户
                }else if (vocation == Vocation.RABBITFARMER) {
                    this.theirJob = new JobLivestockFarmer(this);
                    //牧羊人
