...
public class ContainerWindmill extends Container {
    ...
    public ContainerWindmill(InventoryPlayer playerInventory, IInventory furnaceInventory) {
        ...
        //将插槽添加到容器里
        this.addSlotToContainer(new Slot(furnaceInventory, 0, 56, 22));
        //燃料 风车不用
        //this.addSlotToContainer(new SlotFurnaceFuel(furnaceInventory, 1, 56, 53));
        this.addSlotToContainer(new SlotFurnaceOutput(playerInventory.player, furnaceInventory, 1, 100, 25));
        //三排 一排九个
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 9; ++j) {
                this.addSlotToContainer(new Slot(playerInventory, j + i * 9 + 9, 8 + j * 18, 64 + i * 18));
            }
        }
        //物品栏一排 九个
        for (int k = 0; k < 9; ++k) {
