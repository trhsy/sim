...
//手持镐子
            ...
            //矿工
            this.jobName = new TextComponentTranslation("container.sim.Vocation4",new Object[0]).getUnformattedText();
            this.mine = mb;
            ...
            if (this.atWork) {
                ...
                if (ModSimLoader.money > 0.02F) {
                    ...
                    if (this.stuckItem != null) {
                        //不行,箱子都满了
                        this.folk.setStatus(new TextComponentTranslation("container.sim.job.dairy.farmer.chests",new Object[0]).getUnformattedText());
    ...
    public void mine() {
        try {
                ...
                for(int z = 0; z < this.mine.z; ++z) {
                    for(int x = 0; x < this.mine.x; ++x) {
                        ...
                        if (!(b.isAir(iBlockState, this.folk.entity.world, bp)) && b != Blocks.BEDROCK && !(iBlockState.getMaterial().isLiquid())) {
                            //开采
                            this.folk.setStatus(new TextComponentTranslation("container.sim.Mining10",new Object[0]).getUnformattedText() +" " + b.getLocalizedName());
            ...
            if(this.mineCount>=(this.mine.z*this.mine.x)){
                this.folk.fire();
                ModSimLoader.sendChat(new TextComponentTranslation("container.sim.job.miner.farmer.bedrock",new Object[0]).getUnformattedText());
            }
        }catch (Exception e){
            StackTraceElement element = e.getStackTrace()[0];
    ...
    public void addMiningLevel() {
        try {
            ...
            if (b4 != aft) {
                ModSimLoader.sendChat(this.folk.getName() + " "+new TextComponentTranslation("container.sim.jobMiner1",new Object[0]).getUnformattedText()+" " + aft);
            }
        }catch (Exception e){
            StackTraceElement element = e.getStackTrace()[0];
