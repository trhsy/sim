...
public class BuildingBlueprint implements Comparable<BuildingBlueprint> {
     ...
     * 蓝图名称
     */
    public String name = new TextComponentTranslation("container.sim.gui_Folk_Unknown",new Object[0]).getUnformattedText();
    /**
     * 描述
     */
    public String desc = "";
    ...
    public BuildingBlueprint(String bName, String string,String bType) {
        try {
            ...
            try {
                ...
                //数组长度
                this.structure = new Structure[this.length * this.width * this.height];
                //下一行蓝图信息
                ...
                while ((line = sc.nextLine()) != null) {
                    ...
                    for (int var13 = 0; var13 < var33.length; ++var13) {
                        ...
                        if (String.valueOf(c).contentEquals("!")) {
                            ...
                            if (this.name.contentEquals(new TextComponentTranslation("container.sim.ATMs",new Object[0]).getUnformattedText())) {
                                fs_structure=new Structure();
                                fs_structure.setiBlockState(blockControlBox.getStateFromMeta(1));
                                fs_structure.setMeta(1);
                                this.structure[charNumber] = fs_structure;
                            } else if (new TextComponentTranslation("container.sim.sim_gui_BC_Other",new Object[0]).getUnformattedText().equals(this.buildingType) || new TextComponentTranslation("container.sim.sim_gui_BC_special",new Object[0]).getUnformattedText().equals(this.buildingType)) {
                                fs_structure=new Structure();
                            ...
                            for (BuildingSymbol bs :this.blocks){
                                if (bs.symbol.equals(symbol)) {
                                    ...
                                    fs_structure=new Structure();
                                    fs_structure.setiBlockState(blockstate);
                                    fs_structure.setMeta(bs.meta);
                                    this.structure[charNumber] = fs_structure;
                                    ++charNumber;
                                    break;
                                }
                ...
                if (!hasControlBox) {
                    fs_structure=new Structure();
                    fs_structure.setiBlockState(BlockLoader.blockControlBox.getDefaultState());
                    fs_structure.setMeta(0);
                    this.structure[0] = fs_structure;
                }
...
//            if(!file.getName().contains("new")){
            ...
            try {
                ...
                if (file.getAbsolutePath().contains("residential")) {
                    this.buildingType = new TextComponentTranslation("container.sim.sim_gui_BC_Residential",new Object[0]).getUnformattedText();
                } else if (file.getAbsolutePath().contains("commercial")) {
                    this.buildingType = new TextComponentTranslation("container.sim.sim_gui_BC_Commercial",new Object[0]).getUnformattedText();
                } else if (file.getAbsolutePath().contains("industrial")) {
                    this.buildingType = new TextComponentTranslation("container.sim.sim_gui_BC_Industrial",new Object[0]).getUnformattedText();
                } else if (file.getAbsolutePath().contains("other")) {
                    this.buildingType = new TextComponentTranslation("container.sim.sim_gui_BC_Other",new Object[0]).getUnformattedText();
                } else if (file.getAbsolutePath().contains("special")) {
                    this.buildingType = new TextComponentTranslation("container.sim.sim_gui_BC_special",new Object[0]).getUnformattedText();
                    ...
                    this.buildingType = new TextComponentTranslation("container.sim.sim_gui_BC_Decorative",new Object[0]).getUnformattedText();
                }
...
//                        bw.close();
    ...
    public Hashtable<Item, Integer> getBuildingRequirements() {
        ...
        for (Structure structure:this.structure){
            if (structure != null) {
                Block blockId = structure.getiBlockState().getBlock();
                //检查是否是必须的块
                if (this.isRequiredBlock(blockId)) {
                    Item item = Item.getItemFromBlock(structure.getiBlockState().getBlock());
    ...
    public String getBuildingRequirementsString() {
        ...
        if (this.getBuildingRequirements() != null && this.getBuildingRequirements().size() > 0) {
            ...
            //所需的块
            b.append(new TextComponentTranslation("container.sim.render_1",new Object[0]).getUnformattedText() + this.name + ";");
                ...
                while (it.hasNext()) {
                    try {
                        ...
                        if (pairs.getValue() != null) {
                            ...
                            if ((int) stacks == 0) {
                                //小于1堆
                                ss = new TextComponentTranslation("container.sim.render_2",new Object[0]).getUnformattedText();
                            } else if ((int) stacks == 1) {
                                //1个堆
                                ss = new TextComponentTranslation("container.sim.render_3",new Object[0]).getUnformattedText();
                            } else {
                                //堆
                                ss = (int) stacks + new TextComponentTranslation("container.sim.render_4",new Object[0]).getUnformattedText();
                            }
                            ...
                            if (quantity > 0) {
                                ...
                                //橡木
                                if (itemName.toLowerCase().contentEquals(new TextComponentTranslation("container.sim.sim_gui_BC9",new Object[0]).getUnformattedText())) {
                                    //原木
                                    itemName = new TextComponentTranslation("container.sim.sim_gui_BC10",new Object[0]).getUnformattedText();
                                }
                                //橡木木板
                                if (itemName.toLowerCase().contains(new TextComponentTranslation("container.sim.sim_gui_BC11",new Object[0]).getUnformattedText())) {
                                    //木板
                                    itemName = new TextComponentTranslation("container.sim.sim_gui_BC12",new Object[0]).getUnformattedText();
                                }
            ...
            //暂时没有其他需求
            b.append(new TextComponentTranslation("container.sim.render_No_further_requirements",new Object[0]).getUnformattedText());
        }
        return b.toString();
    }
