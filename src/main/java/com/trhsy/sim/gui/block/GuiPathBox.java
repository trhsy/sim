...
public class GuiPathBox extends GuiScreen {
    ...
    public void initGui() {
        try {
            ...
            GuiButton b=new GuiButton(2, this.width / 2 - 100, 160, new TextComponentTranslation("container.sim.sim_gui_Set_new",new Object[0]).getUnformattedText());
            //置新的快递点/传送点
            this.buttonList.add(b);
            this.theGuiTextField1 = new GuiTextField(0,this.fontRenderer, this.width / 2 - this.width / 3 / 2, 138, this.width / 3, 20);
    ...
    public void drawScreen(int i, int j, float f) {
        try {
            ...
            //路径箱
            this.drawCenteredString(this.fontRenderer, new TextComponentTranslation("container.sim.PathBox4",new Object[0]).getUnformattedText(), this.width / 2, 17, 16777215);
        ...
        try {
            if (guibutton.enabled) {
                if (guibutton.id == 0) {
                    ...
                    //设置新的快递点/传送点
                    if (guibutton.displayString.contentEquals(new TextComponentTranslation("container.sim.sim_gui_Set_new",new Object[0]).getUnformattedText())) {
                        String name = this.theGuiTextField1.getText().trim();
                        if (name.length() == 0) {
                            //请输入此快递点/发送点的名称
                            this.errorText =new TextComponentTranslation("container.sim.Markers5",new Object[0]).getUnformattedText();
                        ...
                        if (chestInvs.size() == 0) {
                            //错误：将至少一个箱子靠近标记。
                            this.errorText = new TextComponentTranslation("container.sim.Markers6",new Object[0]).getUnformattedText();
                            return;
                        }
                        ...
                        for (int p = 0; p < ModSimLoader.theCourierPoints.size(); ++p) {
                            ...
                            if (epoint.equals(point)) {
                                //错误：名称必须是唯一的,' ' 已被使用。
                                this.errorText = new TextComponentTranslation("container.sim.Markers7",new Object[0]).getUnformattedText() + point.toString() + new TextComponentTranslation("container.sim.Markers8",new Object[0]).getUnformattedText();
                                return;
                            }
                        }
                        ...
                        //添加了快递点/传送点'
                        this.errorText = new TextComponentTranslation("container.sim.Markers9",new Object[0]).getUnformattedText() + name + new TextComponentTranslation("container.sim.Markers10",new Object[0]).getUnformattedText();
                    }
                }
            }
        } catch (Exception e) {
