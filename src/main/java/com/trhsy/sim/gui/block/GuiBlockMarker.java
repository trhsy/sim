            ...
            try {
                ...
                if (ftbCountx != 0 && ltrCountx != 0) {
                    ...
                    if (cz == ez) {
                        ...
                        if (cx != ex) {
                            //请面向主标记站立,后方标记在远处。
                            GuiBlockMarker.this.errorText = I18n.format("container.sim.Markers11");
                            //无法复制建筑,技术信息cx:
                            ModSimLoader.sendChat(I18n.format("container.sim.Markers12") + cx + ", cz=" + cz + ", ex=" + ex + ", ez=" + ez);
                            return;
                        }
...
//                                int id = Block.getiDFromBlock(blocks.getBlock());
                    ...
                    if (!check.exists()) {
                        ModSimLoader.sendChat(ModSimLoader.getSimFolder() + "/buildings/ " + I18n.format("container.sim.Markers14"));
                        return;
                    }
                    ...
                    GuiBlockMarker.this.mc.theWorld.playSound(GuiBlockMarker.this.mc.thePlayer,GuiBlockMarker.this.location.x, GuiBlockMarker.this.location.y, GuiBlockMarker.this.location.z, soundEvent, SoundCategory.BLOCKS, 1, 1);
                } else {
