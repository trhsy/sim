...
public class FolkData implements Serializable {
    ...
    public FolkData.FolkAction action;
    public FolkData.FolkAction actionArrival;
    ...
    public transient FolkData.GotoMethod gotoMethod;
    ...
    public FolkData() {
        this.action = FolkData.FolkAction.WANDER;
    ...
    public void onUpdate() {
        ...
        if (now - this.timeSinceLastMinute > 60000L) {
            ...
            if (this.action == FolkData.FolkAction.ATHOME || this.action == FolkData.FolkAction.STAYINGHOME) {
                ...
                try {
                    ...
                    if (this.location.getDistanceTo(liveAt) > 5 && this.destination == null || this.location.theDimension != this.getHome().primaryXYZ.theDimension) {
                        ...
                        if (liveAt != null) {
                            this.gotoXYZ(liveAt, FolkData.GotoMethod.WALK);
                        }
                    }
            ...
            if (this.action == FolkData.FolkAction.WANDER && this.isSpawned() && this.employedAt == null && this.age >= 18 && !this.statusText.contains("baby")) {
                int xo;
                for(xo = 0; xo < ModSimukraft.theBuildings.size(); ++xo) {
                    ...
                    if (b.type.contentEquals("commercial") && dist < 40.0D) {
                        ...
                        if (hasShopKeeper) {
                            ...
                            this.gotoXYZ(b.primaryXYZ, FolkData.GotoMethod.WALK);
                        ...
                        if (b.type.contentEquals("residential") && dist < 40.0D && this.hangingWith == null && b.tennants != null && b.tennants.size() > 0) {
                            ...
                            try {
                                if (!resy.name.contentEquals(this.name) && resy.hangingWith == null && (resy.action == FolkData.FolkAction.WANDER || resy.action == FolkData.FolkAction.STAYINGHOME)) {
                                    ModSimukraft.log.info("FolkData:onUpdate() " + this.name + " is wandering to " + b.displayName + " " + dist + " blocks away.");
                                    this.gotoXYZ(b.primaryXYZ, FolkData.GotoMethod.WALK);
                                    gotWanderPoint = true;
                                    this.statusText = "Hanging out with " + resy.name;
                                    resy.gotoXYZ(b.primaryXYZ, FolkData.GotoMethod.WALK);
                ...
                if (!gotWanderPoint) {
                    ...
                    this.gotoXYZ(wanderTo, FolkData.GotoMethod.WALK);
                    if (this.destination != null) {
                        this.destination.doNotTimeout = true;
                    }
            ...
            } else if (this.action == FolkData.FolkAction.WANDER && this.isSpawned() && this.age < 18) {
                male = Relationship.getMotherOf(this);
                if (male != null) {
                    this.gotoXYZ(male.location, (FolkData.GotoMethod)null);
                }
            }
        ...
        if (now - this.timeSinceLastStatusUpdate > 1000L) {
            ...
            if (ModSimukraft.isDayTime() && this.employedAt != null && this.action != FolkData.FolkAction.ONWAYTOWORK && this.action != FolkData.FolkAction.ATWORK && this.pregnancyStage == 0.0F) {
                ModSimukraft.log.info("FolkData: " + this.name + " is going to work");
                this.statusText = "Going to work";
                this.action = FolkData.FolkAction.ONWAYTOWORK;
                this.gotoXYZ(this.employedAt, (FolkData.GotoMethod)null);
                return;
            }
            ...
            if (ModSimukraft.isDayTime() && this.employedAt != null && this.action != FolkData.FolkAction.ATWORK && this.destination == null && this.pregnancyStage == 0.0F) {
                this.statusText = "Going to work";
                this.action = FolkData.FolkAction.ONWAYTOWORK;
                ...
                temp.x = temp.x + 5.0D;
                this.gotoXYZ(temp, FolkData.GotoMethod.SHIFT);
                this.gotoXYZ(this.employedAt, (FolkData.GotoMethod)null);
                return;
            }

            if (this.action == FolkData.FolkAction.ONWAYTOWORK) {
                this.statusText = "Going to work";
                this.stayPut = false;
                if (this.destination == null) {
                    this.gotoXYZ(this.employedAt, (FolkData.GotoMethod)null);
            ...
            if (this.action == FolkData.FolkAction.STAYINGHOME && this.hangingWith == null) {
            ...
            if (this.action == FolkData.FolkAction.HAVINGBABY) {
            ...
            if (ModSimukraft.isDayTime() && this.employedAt == null && this.action == FolkData.FolkAction.ATHOME) {
                this.isWorking = false;
                if ((new Random()).nextInt(4) == 1) {
                    this.statusText = "Staying Home";
                    this.action = FolkData.FolkAction.STAYINGHOME;
                } else {
                    this.statusText = "Wandering";
                    this.action = FolkData.FolkAction.WANDER;
                }
            ...
            if (this.action == FolkData.FolkAction.STAYINGHOME && this.employedAt != null) {
                this.action = FolkData.FolkAction.WANDER;
            }
            ...
            if (!ModSimukraft.isDayTime() && !isSoldier) {
                ...
                if (this.getHome() == null) {
                    ...
                    if (this.gotoMethod == FolkData.GotoMethod.WALK) {
                        this.updateLocationFromEntity();
                    }
                    ...
                    if (liveAt != null) {
                        ...
                        if (chance > 1 && this.destination == null) {
                            this.stayPut = false;
                            this.gotoXYZ(liveAt, (FolkData.GotoMethod)null);
                            this.action = FolkData.FolkAction.GOINGHOME;
                        ...
                        if (chance <= 1 && !this.statusText.contains("baby")) {
                            this.stayPut = true;
                            this.action = FolkData.FolkAction.ATHOME;
            ...
            if (this.action == FolkData.FolkAction.WANDER) {
                this.stayPut = false;
            }
                    ...
                    if (male != null) {
                        ...
                        if (this.isSpawned()) {
                            ...
                            if ((double)this.matingStage < 0.15D) {
                                this.gotoXYZ(male.location, FolkData.GotoMethod.SHIFT);
                            }
    ...
    private void tryForBaby() {
        if (this.gender == 1 && this.pregnancyStage == 0.0F) {
            ...
            if (malePartner != null && this.action == FolkData.FolkAction.ATHOME && malePartner.action == FolkData.FolkAction.ATHOME) {
                this.matingStage = 0.0F;
                if (malePartner.isSpawned()) {
                    this.gotoXYZ(new V3(malePartner.theEntity.field_70165_t, malePartner.theEntity.field_70163_u, malePartner.theEntity.field_70161_v, malePartner.theEntity.field_71093_bK), FolkData.GotoMethod.WALK);
                }
            }
        } else {
            this.matingStage = -1.0F;
        ...
        if (this.action == FolkData.FolkAction.WANDER) {
            ...
            for(int b = 0; b < ModSimukraft.theBuildings.size(); ++b) {
                ...
                if (building.tennants.size() == 0 && building.buildingComplete && building.type.contentEquals("residential")) {
                    ...
                    this.action = FolkData.FolkAction.GOINGHOME;
                    this.actionArrival = FolkData.FolkAction.STAYINGHOME;
                    if (building.livingXYZ != null) {
                        this.gotoXYZ(building.livingXYZ, (FolkData.GotoMethod)null);
                    } else {
                        this.gotoXYZ(building.primaryXYZ, (FolkData.GotoMethod)null);
                    }
    ...
    public void gotoXYZ(V3 whereTo, FolkData.GotoMethod methodOfTravel) {
        if (whereTo != null) {
            this.stayPut = false;
            ...
            if (this.destination != null) {
                ...
                if (methodOfTravel == null) {
                    ...
                    if (dist < 40) {
                        this.gotoMethod = FolkData.GotoMethod.WALK;
                    }

                    if (!this.isSpawned() || dist >= 40) {
                        this.gotoMethod = FolkData.GotoMethod.BEAM;
                    }

                    if (playpos != null) {
                        if (this.location.getDistanceTo(playpos) >= 40 && whereTo.getDistanceTo(playpos) >= 40) {
                            this.gotoMethod = FolkData.GotoMethod.SHIFT;
                        }
                    ...
                    if (methodOfTravel == null) {
                        methodOfTravel = FolkData.GotoMethod.SHIFT;
                    }
                } else {
                ...
                if (this.destination != null) {
                    if (this.gotoMethod == FolkData.GotoMethod.SHIFT) {
                        ...
                        this.destination = null;
                    } else if (this.gotoMethod == FolkData.GotoMethod.BEAM) {
                        this.timeStartedGotoing = System.currentTimeMillis();
                        this.beamMeTo(whereTo);
                    } else if (this.gotoMethod == FolkData.GotoMethod.WALK) {
                        this.stayPut = false;
                        this.timeStartedGotoing = System.currentTimeMillis();
                        if (this.theEntity != null) {
