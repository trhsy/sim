...
public class JobDairyFarmer extends Job {
    ...
    public JobDairyFarmer(FolkData folk) {
        try {
            ...
            ModSimReloaded.log.error("JobDairyFarmer出错了：" + e.getMessage());
        }
    }
    ...
    private void createCowNames() {
        try {
            ...
            ModSimReloaded.log.error("createCowNames出错了：" + e.getMessage());
        }
    }
    ...
    private void stageArrived() {
        try {
            ...
            this.theFolk.statusText = I18n.func_135052_a("container.sim.job.dairy.farmer.Warming");
            //int count = false;
    ...
    private void stageWaiting() {
        try {
            ...
            this.theFolk.statusText = I18n.func_135052_a("container.sim.job.dairy.farmer.Sterilizing");
    ...
    private void stageMilking() {
        try {
            ...
            this.theFolk.statusText = I18n.func_135052_a("container.sim.job.dairy.farmer.Milking") + this.cowNames[c] + I18n.func_135052_a("container.sim.job.dairy.farmer.the_cow");
            this.theStage = Stage.STORINGMILK;
            this.theFolk.isWorking = true;
        } catch (Exception e) {
            ModSimReloaded.log.error("stageMilking出错了：" + e.getMessage());
        }
    }

    private void stageStoringMilk() {
        try {
            this.theFolk.statusText = I18n.func_135052_a("container.sim.job.dairy.farmer.Storing");
            this.theFolk.isWorking = false;
            this.farmChests = inventoriesFindClosest(this.theFolk.employedAt, 5);
            if (this.farmChests.size() > 0) {
                boolean ok = this.inventoriesPut(this.farmChests, new ItemStack(Items.field_151117_aB, 1), true);
                if (!ok) {
                    ModSimReloaded.sendChat(this.theFolk.name + I18n.func_135052_a("container.sim.job.dairy.farmer.dairy"));
                    this.theFolk.statusText = I18n.func_135052_a("container.sim.job.dairy.farmer.chests");
                ...
                this.theFolk.statusText = I18n.func_135052_a("container.sim.job.dairy.farmer.Who");
                this.theStage = Stage.CANTWORK;
            }
        } catch (Exception e) {
            ModSimReloaded.log.error("出错了：" + e.getMessage());
    ...
    public void onArrivedAtWork() {
        try {
            ...
            if (dist <= 1) {
                ...
                this.theFolk.statusText = I18n.func_135052_a("container.sim.job.dairy.farmer.Arrived");
    ...
    private void spawnCows(V3 controlBox, int count) {
        ...
        try {
            ...
            ModSimReloaded.log.error("spawnCows出错了：" + e.getMessage());
        }
    }
}
