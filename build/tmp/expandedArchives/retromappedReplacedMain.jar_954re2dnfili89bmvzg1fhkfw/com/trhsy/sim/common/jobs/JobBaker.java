...
public class JobBaker extends Job implements Serializable {
    ...
    public JobBaker(FolkData folk) {
        try {
            ...
            StackTraceElement element=e.getStackTrace()[0];ModSimReloaded.log.error("JobBaker出错了：" + e.getMessage()+"行数："+element.getLineNumber());
        }
    ...
    private void stageGoingToWheatFarm() {
        try {
            this.theFolk.statusText = I18n.func_135052_a("container.sim.job.Baker_Fetching");
            ...
            StackTraceElement element=e.getStackTrace()[0];ModSimReloaded.log.error("stageGoingToWheatFarm出错了：" + e.getMessage()+"行数："+element.getLineNumber());
        }
    ...
    private void stageCollectingWheat() {
        try {
            this.theFolk.statusText = I18n.func_135052_a("container.sim.job.Baker_Collecting");
            this.runDelay = 1000;
            if (this.step == 1) {
                this.farmChests = inventoriesFindClosest(this.farm.getLocation(), 5);
                if (this.farmChests.size() > 0) {
                    ((IInventory)this.farmChests.get(0)).func_174889_b(mc.field_71439_g);
                    this.step = 2;
                }
            } else if (this.step == 2) {
                this.farmChests = inventoriesFindClosest(this.farm.getLocation(), 5);
                this.inventoriesTransferToFolk(this.theFolk.getVillagerInventory(), this.farmChests, new ItemStack(Items.field_151015_O, 640), Blocks.field_150350_a);
                this.step = 3;
            } else if (this.step == 3) {
                ((IInventory)this.farmChests.get(0)).func_174886_c(mc.field_71439_g);
                this.theStage = Stage.GOINGTOWHEATFARM;
                this.step = 1;
            }
        } catch (Exception e) {
            StackTraceElement element=e.getStackTrace()[0];ModSimReloaded.log.error("stageCollectingWheat出错了：" + e.getMessage()+"行数："+element.getLineNumber());
        }
    }
    ...
    private void stageGoBackToBakery() {
        try {
            ...
            StackTraceElement element=e.getStackTrace()[0];ModSimReloaded.log.error("stageGoBackToBakery出错了：" + e.getMessage()+"行数："+element.getLineNumber());
        }
    }
    ...
    private void stageMakeBread() {
        try {
            this.theFolk.statusText = I18n.func_135052_a("container.sim.job.Baker_Baking_bread");
            ...
            if (this.bakeryChests != null && this.bakeryChests.size() != 0) {
                ...
                if (this.step == 1) {
                    ...
                    try {
                        ((IInventory)this.bakeryChests.get(0)).func_174889_b(mc.field_71439_g);
                    } catch (Exception e) {
                    }
                    ...
                    ((IInventory) this.bakeryChests.get(0)).func_174886_c(mc.field_71439_g);
                    this.theFolk.statusText = I18n.func_135052_a("container.sim.job.Baker_Selling");
                    this.theFolk.stayPut = true;
                    if (this.theFolk.theEntity != null) {
                ...
                this.theFolk.statusText = I18n.func_135052_a("container.sim.job.Baker_Who");
            }
        } catch (Exception e) {
            StackTraceElement element=e.getStackTrace()[0];ModSimReloaded.log.error("stageMakeBread出错了：" + e.getMessage()+"行数："+element.getLineNumber());
        }
    ...
    private void stageSellingBread() {
        try {
            if (this.step == 1) {
                ...
                this.theFolk.statusText = I18n.func_135052_a("container.sim.job.Baker_Closing");
                //int sell = false;
                ...
                if (breadStack == null) {
                    ModSimReloaded.sendChat(this.theFolk.name + I18n.func_135052_a("container.sim.job.Baker_today"));
                } else {
            ...
            StackTraceElement element=e.getStackTrace()[0];ModSimReloaded.log.error("stageSellingBread出错了：" + e.getMessage()+"行数："+element.getLineNumber());
        }
    }
    ...
    public void onArrivedAtWork() {
        ...
        try {
            ...
            if (dist <= 1) {
                ...
                this.theFolk.stayPut = true;
                this.theFolk.statusText = I18n.func_135052_a("container.sim.job.Baker_Arrived");
