...
public class JobCheesemaker extends Job {
    ...
    public JobCheesemaker(FolkData folk) {
        try {
            ...
            ModSimReloaded.log.error("JobCheesemaker出错了：" + e.getMessage());
        }
    ...
    public void onUpdate() {
        try {
            ...
            if (this.theCheeseFactory == null) {
                ...
                ModSimReloaded.sendChat(I18n.func_135052_a("container.sim.job.cheese_maker.There"));
            } else {
                //是晚上，状态设置为闲置
    ...
    private void stageArrivedAtFactory() {
        try {
            ...
            this.inventoriesTransferToFolk(this.theFolk.getVillagerInventory(), chests, new ItemStack(Items.field_151117_aB, 64), (Block)null);
            this.theStage = Stage.GOINGTODAIRYFARM;
            this.currentFarmNum = -1;
        } catch (Exception e) {
            ModSimReloaded.log.error("stageArrivedAtFactory出错了：" + e.getMessage());
        }
    }
    ...
    private void stageGoingToDairyFarm() {
        try {
            //去收集牛奶
            this.theFolk.statusText = I18n.func_135052_a("container.sim.job.cheese_maker.Going");
            //当前农场数量
            ++this.currentFarmNum;
            //通过搜索获得建筑 奶牛场
            ArrayList<Building> dairyFarms = Building.getBuildingBySearch(I18n.func_135052_a("container.sim.gui_contains_Dairy_Farm"), true);
            ...
            if (!dairyFarms.isEmpty() && dairyFarms.size() - 1 <= this.currentFarmNum) {
                ...
                //否则 没有检测到奶牛场，员工自动退休
                ModSimReloaded.sendChat(this.theFolk.name + I18n.func_135052_a("container.sim.job.cheese_maker.has_retired"));
            ...
            ModSimReloaded.log.error("stageGoingToDairyFarm出错了：" + e.getMessage());
        }
    }
    ...
    private void stageCollectingMilk() {
        try {
            //收集牛奶
            this.theFolk.statusText = I18n.func_135052_a("container.sim.job.cheese_maker.Collecting");
            if (this.step == 1) {
                //如果npc 目的地为空 并且距离小于5
                ...
                if (this.chestsAtDairy.isEmpty()) {
                    //当前位置在奶牛场找不到箱子，我辞职了！
                    ModSimReloaded.sendChat(this.theFolk.name + I18n.func_135052_a("container.sim.job.cheese_maker.I_quit"));
                    this.theFolk.selfFire();
                    return;
                }
                //库存转移到民间 牛奶场的箱子                                                       牛奶桶
                this.inventoriesTransferToFolk(this.theFolk.getVillagerInventory(), this.chestsAtDairy, new ItemStack(Items.field_151117_aB, 1), (Block)null);
                //如果npc 库存为空
                if (this.theFolk.getVillagerInventory() == null) {
                    //今天在牛奶场没找到牛奶。
                    ModSimReloaded.sendChat(this.theFolk.name + I18n.func_135052_a("container.sim.job.cheese_maker.dairy"));
                    //设置为切奶酪
                    this.theStage = Stage.SLICECHEESE;
                    this.step = 1;
                    this.theFolk.isWorking = false;
                    //没有牛奶可加工，今天会很轻松！
                    this.theFolk.statusText = I18n.func_135052_a("container.sim.job.cheese_maker.process");
                    return;
                }
                //奶酪厂 获取特除方块 3
            ...
            ModSimReloaded.log.error("stageCollectingMilk出错了：" + e.getMessage());
        }
    }
    /**
     * @Author fan
        ...
        try {
            if (this.step == 1) {
                if (this.theFolk.destination == null) {
                    ...
                    this.step = 2;
                    //准备加满水槽
                    this.theFolk.statusText = I18n.func_135052_a("container.sim.job.cheese_maker.Preparing");
                } else {
            ...
            ModSimReloaded.log.error("stageGoingToTank出错了：" + e.getMessage());
        }
    }
    /**
     * @Author fan
    ...
    private void stageEmptyingMilk() {
        try {
            if (this.step == 1) {
                ...
                if (this.theFolk.getVillagerInventory() != null) {
                    //库存大于1
                    if (this.theFolk.getVillagerInventory().func_70302_i_() > 1) {
                        //倒了 N 桶牛奶
                        this.theFolk.statusText = I18n.func_135052_a("container.sim.job.cheese_maker.Emptying") + this.theFolk.getVillagerInventory().func_70302_i_() + I18n.func_135052_a("container.sim.job.cheese_maker.buckets");
                    } else {
                        //倒了所有的牛奶
                        this.theFolk.statusText = I18n.func_135052_a("container.sim.job.cheese_maker.Emptied");
                    }
            ...
            ModSimReloaded.log.error("stageEmptyingMilk出错了：" + e.getMessage());
        }
    ...
    private void stageStiring() {
        try {
            ...
            if (this.step == 1) {
                //检查牛奶粘度
                this.theFolk.statusText = I18n.func_135052_a("container.sim.job.cheese_maker.viscosity");
                if (!stirPositions.isEmpty()) {
                    ...
                    ModSimReloaded.sendChat(I18n.func_135052_a("container.sim.job.cheese_maker.constructor"));
                    this.theFolk.selfFire();
                }
            } else if (this.step == 2) {
                if (this.theFolk.destination == null) {
                ...
                switch(this.stirCount) {
                    case 0:
                        //搅拌牛奶
                        say = I18n.func_135052_a("container.sim.job.cheese_maker.Stirring");
                        break;
                    case 1:
                        //加入绝密成分
                        say = I18n.func_135052_a("container.sim.job.cheese_maker.ingredient");
                        break;
                    case 2:
                        //添加细菌培养
                        say = I18n.func_135052_a("container.sim.job.cheese_maker.bacterial");
                        break;
                    case 3:
                        //除去不需要的孢子
                        say = I18n.func_135052_a("container.sim.job.cheese_maker.unwanted");
                        break;
                    case 4:
                        //检查发酵进度
                        say = I18n.func_135052_a("container.sim.job.cheese_maker.fermentation");
                        break;
                    case 5:
                        //加入凝乳酶
                        say = I18n.func_135052_a("container.sim.job.cheese_maker.Adding");
                        break;
                    case 6:
                        //网条状奶酪
                        say = I18n.func_135052_a("container.sim.job.cheese_maker.Reticulating");
                }
            ...
            ModSimReloaded.log.error("stageStiring出错了：" + e.getMessage());
        }
        ...
        try {
            ...
            if (!milkBlocks.isEmpty() && !cheeseBlocks.isEmpty()) {
                ...
                for(int m = milkBlocks.size() - 1; m > 0; --m) {
                    ...
                    id = this.jobWorld.func_180495_p(blockPos).func_177230_c();
                    dist = id.func_176201_c(this.jobWorld.func_180495_p(blockPos));

                    if (id == BlockLoader.blockFluidMilk && dist == 0) {
                        this.jobWorld.func_180501_a(blockPos,id.func_176223_P(),3);
                        ++milkGotCount;
                        if (milkGotCount > 1) {
                            break;
                        }
                ...
                if (milkGotCount > 0) {
                    ...
                    while(i$.hasNext()) {
                        cheese = (V3) i$.next();
                        id = this.jobWorld.func_180495_p(new BlockPos(cheese.x.intValue(), cheese.y.intValue(), cheese.z.intValue())).func_177230_c();
                        dist = cheese.getDistanceTo(currentStirPos);
                        if (id != BlockLoader.blockCheese && dist < 5) {
                            BlockPos blockPos=new BlockPos(cheese.x.intValue(), cheese.y.intValue(), cheese.z.intValue());
                            this.jobWorld.func_180501_a(blockPos,BlockLoader.blockCheese.func_176223_P(),3);
                            placedCheese = true;
                            break;
                        }
                    }
                ...
                ModSimReloaded.sendChat(I18n.func_135052_a("container.sim.job.cheese_maker.Cheese_factory"));
            }
        } catch (Exception e) {
            ModSimReloaded.log.error("transformMilkToCheese出错了：" + e.getMessage());
        }
    ...
    private void stageHarvestCheese() {
        try {
            ...
            ArrayList<V3> stirPositions = this.theCheeseFactory.getSpecialBlocks(4);
            //提取奶酪块
            this.theFolk.statusText = I18n.func_135052_a("container.sim.job.cheese_maker.Extracting");
            if (this.step == 1) {
                ...
                if (this.step == 3) {
                    ...
                    while(iterator.hasNext()) {
                        ...
                        id = this.jobWorld.func_180495_p(new BlockPos(block.x.intValue(), block.y.intValue(), block.z.intValue())).func_177230_c();
                        if (((V3) stirPositions.get(0)).getDistanceTo(block) < 5 && id == BlockLoader.blockCheese) {
                            gotBlock = true;
                            this.theFolk.getVillagerInventory().func_70299_a(0,new ItemStack(BlockLoader.blockCheese));
                    ...
                    if (!gotBlock) {
                        ...
                        this.theStage = Stage.SLICECHEESE;
                        //计算奶酪块
                        this.theFolk.statusText = I18n.func_135052_a("container.sim.job.cheese_maker.Counting");
                    }
                }
            }
        } catch (Exception e) {
            ModSimReloaded.log.error("stageHarvestCheese出错了：" + e.getMessage());
        }
    ...
    private void stageSliceCheese() {
        try {
            ...
            if (slicewaypoint.isEmpty()) {
                ...
                ModSimReloaded.sendChat(I18n.func_135052_a("container.sim.job.cheese_maker.problem"));
            } else {
                if (this.step == 1) {
                    ...
                    if (this.step == 3) {
                        ...
                        if (chests.isEmpty()) {
                            //有人在奶酪工厂箱子里偷奶酪，我不干了！
                            ModSimReloaded.sendChat(this.theFolk.name + I18n.func_135052_a("container.sim.job.cheese_maker.Someone"));
                            this.theFolk.selfFire();
                        }
                        ...
                        chests = Job.inventoriesFindClosest((V3) slicewaypoint.get(0), 4);
                        //奶酪工厂的箱子装满了奶酪！
                        this.theFolk.statusText = I18n.func_135052_a("container.sim.job.cheese_maker.Slicing");
                        ...
                        if (cheese != null) {
                            ...
                            if (!placedOK) {
                                ModSimReloaded.sendChat(this.theFolk.name + I18n.func_135052_a("container.sim.job.cheese_maker.factory"));
                                this.theFolk.selfFire();
                            }
                        } else {
                            this.step = 5;
                        ...
                        //我喜欢的奶酪！
                        this.theFolk.statusText = I18n.func_135052_a("container.sim.job.cheese_maker.love_cheese");
                    }
                }

            }
        } catch (Exception e) {
            ModSimReloaded.log.error("stageSliceCheese出错了：" + e.getMessage());
        }
