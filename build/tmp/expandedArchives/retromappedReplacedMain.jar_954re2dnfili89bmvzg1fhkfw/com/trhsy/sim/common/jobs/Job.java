...
public abstract class Job {
    ...
    public ArrayList<ItemStack> translateBlockWhenMined(World world, V3 location) {
        ...
        try {
            ...
            itemStacks = (ArrayList<ItemStack>) block.getDrops(world, blockPos, block.func_176203_a(ma), 0);
        } catch (Exception e) {
    ...
    public static V3 findClosestBlockType(V3 startXYZ, Block block, int searchDistance, boolean mustSeeSky) {
        ...
        try {
            World theWorld = MinecraftServer.func_71276_C().func_71218_a(startXYZ.theDimension);
            if (theWorld.func_180495_p(new BlockPos(startXYZ.x.intValue(), startXYZ.y.intValue(), startXYZ.z.intValue())).func_177230_c() == block) {
                return startXYZ;
            } else {
                for (int d = 1; d < searchDistance; d++) {
    ...
    public int getAnimalCountInPen(V3 controlBox, Class animal) {
        int size = 0;
        try {
            List list = this.jobWorld.func_72872_a(animal, new AxisAlignedBB(controlBox.x, controlBox.y, controlBox.z, controlBox.x + 1, controlBox.y + 1, controlBox.z + 1).func_72314_b(3.0, 2.0, 3.0));
            if (list == null) {
                return size;
            } else {
                size = list.size();
