...
public abstract class Job {
    ...
    public void onUpdate() {
        try {
            ...
            if (this.chestToClose != null && System.currentTimeMillis() > this.chestToCloseWhen) {
                //关上箱子
                this.chestToClose.func_174886_c(this.mc.field_71439_g);
                //设置为空
        ...
        try {
            ...
            if (inventoryBasic != null) {
                ...
                for (int i = 0; i < inventoryBasic.func_70302_i_(); i++) {
                    //当前方块的数量
                    ItemStack is = inventoryBasic.func_70301_a(i);
                    if (is != null) {
    ...
    public int getInventoryCount(FolkData theFolk, Block item) {
        ...
        try {
            String lang= FMLCommonHandler.instance().getCurrentLanguage();
            //循环库存
            for (int i = 0; i < theFolk.getVillagerInventory().func_70302_i_(); i++) {
                //当前方块的数量
                ItemStack is = (ItemStack) theFolk.getVillagerInventory().func_70301_a(i);
                if (is != null) {//如果物品 对上
                    String name=is.func_82833_r();
                    String names=new ItemStack(item).func_82833_r();
                    if("en_US".equals(lang)){
    ...
    public TileEntityFurnace findFurnace(V3 v) {
        ...
        try {
            ...
            //如果等于空重新赋值
            if (vRet == null) {
                vRet = findClosestBlockType(v, Blocks.field_150470_am, 5, false);
            }
            //如果不为空
            if (vRet != null) {
                //世界服务器的维度                                                       维度
                World theWorld = MinecraftServer.func_71276_C().func_71218_a(vRet.theDimension);
                BlockPos blockPos = new BlockPos(vRet.field_72450_a, vRet.field_72448_b, vRet.field_72449_c);
                //设置熔炉位置 转换为int
                ret = (TileEntityFurnace) theWorld.func_175625_s(blockPos);
            }
        } catch (Exception e) {
                ...
                for (int i = 0; i < chest.func_70302_i_(); i++) {
                    ...
                    if (is == null) {
                        ...
                        if (isTest != null) {
                            ...
                            ModSimReloaded.log.warn("Job: placeIntoInventory() 无法将 " + is.func_82833_r() + " 放入空槽 " + i);
                            placedOK = false;
                        }
                    } else if (is.func_77973_b() == inStack.func_77973_b() && is.field_77994_a < is.func_77976_d()) {
                        int isBefore = chest.func_70301_a(i).field_77994_a;
                        is.field_77994_a++;
                        //将给定的物品堆栈设置为库存中的指定位置（可以是工艺或装甲部分）。
                        chest.func_70299_a(i, is);
                        int isAfter = chest.func_70301_a(i).field_77994_a;
                        if (isAfter > isBefore) {
                            placedOK = true;
                            break;
                        } else {
                            ModSimReloaded.log.warn("Job: placeIntoInventory() 无法更改大小 " + is.func_82833_r() + " in slot " + i);
                            placedOK = false;
                        }
                    }
                }
            ...
            if (whatItem == null) {
                ...
                for (int i = 0; i < chest.func_70302_i_(); i++) {
                    ...
                    if (chestStack != null && ignore == false) {
                        if (!compareMeta) {
                            chestStack.func_77964_b(whatItem.func_77952_i());
                        }
    ...
    public boolean inventoriesTransferFromFolk(InventoryBasic folkInventory, List<IInventory> toChests, ItemStack specificItems) {
        ...
        try {
            for (int i = 0; i < folkInventory.func_70302_i_(); i++) {
                try {
                    ItemStack folkStack = (ItemStack) folkInventory.func_70301_a(i);
                    ...
                    if (okToPlace) {
                        ...
                        if (!placed) {
                            ModSimReloaded.log.warn("Job: 无法放置一堆 " + folkStack.func_82833_r() + " in chest");
                            return false;
                        }
                    }
                } catch (Exception e) {
    ...
    public boolean inventoriesTransferToFolk(InventoryBasic folkInventory, List<IInventory> fromChests, ItemStack whatItems, Block ignoreId) {
        ...
        try {
            ...
            do {
                ...
                if (got != null) {
                    folkInventory.func_70299_a(limit, got);
                    ret = true;
                }
                ...
                for (int g = 0; g < chest.func_70302_i_(); g++) {
                    ...
                    if (gotMatch) {
                        while (gotSoFar < getQty && chestStack.field_77994_a > 0) {
                            gotSoFar++;
                            chestStack.field_77994_a--;
                            folkInventory.func_70299_a(0, new ItemStack(Block.func_149634_a(chestStack.func_77973_b()), 1, chestStack.func_77960_j()));
                        }
    ...
    public List<ItemStack> translateBlockWhenMined(World world, V3 location) {
        ...
        try {
            int i = (int) location.field_72450_a;
            int j = (int) location.field_72448_b;
            int k = (int) location.field_72449_c;
            ...
            int ma = block.func_176201_c(world.func_180495_p(blockPos));
            itemStacks = block.getDrops(world, blockPos, block.func_176203_a(ma), 0);
        } catch (Exception e) {
    ...
    public void openCloseChest(IInventory chest, int msDelay) {
        try {
            chest.func_174889_b(this.mc.field_71439_g);
    ...
    public void setClosestBlocksOfType(final V3 startXYZ, final List<Block> blockIDs, final int distanceLimit, final boolean needsToSeeSky, final boolean scanDownwards, final boolean oneLayerOnly) {
            ...
            Thread t = new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                    World theWorld = MinecraftServer.func_71276_C().func_71218_a(startXYZ.theDimension);
                        ...
                        for (int d = 1; d < distanceLimit; d++) {
                            for (int xo = -d; xo <= d; xo++) {
                                for (int zo = -d; zo <= d; zo++) {
                                    int sx = (int) (startXYZ.field_72450_a + xo);
                                    int sy;
                                    if (scanDownwards) {
                                        sy = (int) (startXYZ.field_72448_b - yo);
                                    } else {
                                        sy = (int) (startXYZ.field_72448_b + yo);
                                    }

                                    int sz = (int) (startXYZ.field_72449_c + zo);
                                    skip = false;
                                    ...
                                    for (int b = 0; b < blockIDs.size(); b++) {
                                        ...
                                        Block blockInWorld = theWorld.func_180495_p(new BlockPos(sx, sy, sz)).func_177230_c();
                                        if (blockInWorld == blockID) {
                                            if (needsToSeeSky) {
                                                boolean canSeeSky;
                                                if (theWorld.func_180495_p(new BlockPos(sx, sy + 1, sz)).func_177230_c() == null) {
                                                    canSeeSky = true;
                                                } else {
                                                    canSeeSky = false;
                                                }
    ...
    public static List<IInventory> inventoriesFindClosest(V3 startXYZ, int searchDistance) {
        ...
        try {
            World theWorld = MinecraftServer.func_71276_C().func_71218_a(startXYZ.theDimension);
            BlockPos blockPos = new BlockPos(startXYZ.field_72450_a, startXYZ.field_72448_b, startXYZ.field_72449_c);
            ...
            for (int d = 1; d < searchDistance; d++) {
                for (int yo = -d; yo <= d; yo++) {
                    for (int xo = -d; xo <= d; xo++) {
                        for (int zo = -d; zo <= d; zo++) {
                            int sx = (int) (startXYZ.field_72450_a + xo);
                            int sy = (int) (startXYZ.field_72448_b + yo);
                            int sz = (int) (startXYZ.field_72449_c + zo);
                            blockPos = new BlockPos(sx, sy, sz);
                            te = theWorld.func_175625_s(blockPos);
                            if (te != null) {
    ...
    public static V3 findAdjacentSpace(V3 startXYZ, World world) {
        ...
        try {
            ...
            BlockPos blockPos = new BlockPos(test.field_72450_a+1, test.field_72448_b, test.field_72449_c);
            if (((World) theWorld).func_175623_d(blockPos)) {
                return test;
            }
            test = startXYZ.clone();
            blockPos = new BlockPos(test.field_72450_a-1, test.field_72448_b, test.field_72449_c);
            if (((World) theWorld).func_175623_d(blockPos)) {
                return test;
            }
            test = startXYZ.clone();
            blockPos = new BlockPos(test.field_72450_a, test.field_72448_b, test.field_72449_c+1);
            if (((World) theWorld).func_175623_d(blockPos)) {
                return test;
            }
            test = startXYZ.clone();
    ...
    public static V3 findClosestBlockType(V3 startXYZ, Block block, int searchDistance, boolean mustSeeSky) {
        ...
        try {
            World theWorld = MinecraftServer.func_71276_C().func_71218_a(startXYZ.theDimension);
            if (theWorld.func_180495_p(new BlockPos(startXYZ.field_72450_a, startXYZ.field_72448_b, startXYZ.field_72449_c)).func_177230_c() == block) {
                return startXYZ;
            } else {
                for (int d = 1; d < searchDistance; d++) {
                    for (int yo = -searchDistance; yo <= searchDistance; yo++) {
                        for (int xo = -d; xo <= d; xo++) {
                            for (int zo = -d; zo <= d; zo++) {
                                ...
                                int sy = (int) (startXYZ.field_72448_b + yo);
                                int sz = (int) (startXYZ.field_72449_c + zo);
                                if (theWorld.func_180495_p(new BlockPos(sx, sy, sz)).func_177230_c() == block) {
    ...
    public static V3 findClosestBlockType(V3 startXYZ, Block block, int searchDistance) {
        ...
        try {
            World theWorld = MinecraftServer.func_71276_C().func_71218_a(startXYZ.theDimension);
            BlockPos blockpos = new BlockPos(startXYZ.field_72450_a, startXYZ.field_72448_b, startXYZ.field_72449_c);
            if (theWorld.func_180495_p(blockpos).func_177230_c() == block) {
                return startXYZ;
            } else {
                for (int d = 1; d < searchDistance; d++) {
                    for (int xo = -d; xo <= d; xo++) {
                        for (int zo = -d; zo <= d; zo++) {
                            int sx = (int) (startXYZ.field_72450_a + xo);
                            int sy = (int) startXYZ.field_72448_b;
                            int sz = (int) (startXYZ.field_72449_c + zo);
                            if (theWorld.func_180495_p(new BlockPos(sx, sy, sz)).func_177230_c() == block) {
                                ret = new V3((double) sx, (double) sy, (double) sz, startXYZ.theDimension);
                                return ret;
    ...
    public static List<V3> findClosestBlocks(V3 startXYZ, Block block, int distanceLimit) {
        ...
        try {
            ...
            for (int i = 0; i < blocksFound.size(); i++) {
                ...
                double distance = Math.sqrt((v.field_72450_a - startXYZ.field_72450_a) * (v.field_72450_a - startXYZ.field_72450_a) + (v.field_72449_c - startXYZ.field_72449_c) * (v.field_72449_c - startXYZ.field_72449_c));
                if (distance < cd) {
                    cd = distance;
                    ci = i;
                }
    ...
    public int getAnimalCountInPen(V3 controlBox, Class animal) {
        int size = 0;
        try {
            List list = this.jobWorld.func_72872_a(animal, new AxisAlignedBB(controlBox.field_72450_a, controlBox.field_72448_b, controlBox.field_72449_c, controlBox.field_72450_a + 1, controlBox.field_72448_b + 1, controlBox.field_72449_c + 1).func_72314_b(3.0, 2.0, 3.0));
            if (list == null) {
                return size;
            } else {
                size = list.size();
